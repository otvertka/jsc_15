...
class App {
  ...
  _newWorkout(e) {
    const validInputs = (...inputs) => inputs.every(inp => Number.isFinite(inp));  // Цикл проходит по всему массиву и проверяет isFinite() функция определяет, является ли переданное значение конечным числом
    e.preventDefault();
    // Get data from form
