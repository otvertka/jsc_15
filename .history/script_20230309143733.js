...
class App {
  ...
  _loadMap(position) {
    ...
    this.#map = L.map('map').setView(coords, this.#mapZoomLevel); // 13 - размер карты на starte
    // console.log(map);
...
`;
  ...
  `;
    ...
    form.insertAdjacentHTML('afterend', html); // вставляем хтмл Активностей
  }
  ...
  _moveToPopup(e) {
    ...
    // workout.click();  // НЕ РАБОТАЕТ, т.к. разрывается цепочка прототипов
  }
  _setLocalStorage() {
    localStorage.setItem('workouts', JSON.stringify(this.#workouts)); // конвертируем в строку
  }

  _getLocalStorage() {
    const data = JSON.parse(localStorage.getItem('workouts')); // конвертируем в объект
    console.log(data);

    if (!data) return;

    this.#workouts = data; // при загрузке страницы Загружаем события из ЛС
